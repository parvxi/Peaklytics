...
async def predict(data: CompanyData):
    try:
        ...
        logging.info(f"Predicted status: {predicted_status}")

        # Perform useful calculations
        calculations = perform_calculations(data)
        logging.info(f"Calculations: {calculations}")

        # Apply rule-based system for additional insights
        rule_based_insights = apply_rule_based_insights(data)
        logging.info(f"Rule-based insights: {rule_based_insights}")

        # Generate prompt for LLM (bypassing LLM for now to isolate the issue)
        prompt = generate_prompt(predicted_status, data, calculations)
        logging.info(f"Generated LLM prompt: {prompt}")

        # Temporarily bypass LLM invocation to debug
